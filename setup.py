...
def force_install(reqs, no_deps=False, optional=False):
    ...
    try:
        ...
        if optional:
            print(f"  [WARNING] Optional package failed to install: {reqs}")
            print(f"           This is not critical. Continuing...")
        else:
            raise
...
def setup_pyla():
    ...
    base_reqs = [
        "PySide6>=6.6.0", "customtkinter>=5.2.0", "toml>=0.10.2", "Pillow>=10.0.0",
        "discord.py>=2.3.2", "opencv-python==4.8.0.76", "requests", "ultralytics",
        "aiohttp", "easyocr", "google-play-scraper", "pyautogui", "packaging"
    ]
    ...
    force_install(["adbutils==2.12.0"])
    # av requires FFmpeg headers - optional on Windows
    force_install(["av==12.3.0"], optional=True)
    # Try scrcpy-client - try main branch first (better Python 3.14 support)
    force_install(["git+https://github.com/leng-yue/py-scrcpy-client.git"], optional=True)
